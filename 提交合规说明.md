...
五题均采用程序化物理抓取作为主路径，并加载该题团队 BC checkpoint。`pick_up.py` 依据 SceneContext 匹配任务并自动选择五个已验证权重，不需要人工替换配置。现有 100/100 是 2026-08-11 的历史评分证据，原文件未改写；2026-08-15 合规边界重构后又完成五题物理回归（5/5 级别、7 条完整四技能链路通过，L5 为 3/3 个物体、12/12 步），但该回归不冒充官方得分，最终仍需主办方复现。
